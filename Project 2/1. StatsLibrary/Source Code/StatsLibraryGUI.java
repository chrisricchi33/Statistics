...
public class StatsLibraryGUI 
{
    ...
    private void createAndShowGUI() 
    {
        ...
        JButton[] buttons = 
        {
            createStyledButton("V1 - Find Mean", Color.LIGHT_GRAY),
            createStyledButton("V1 - Find Median", Color.RED),
            createStyledButton("V1 - Find Mode", Color.GREEN),
            createStyledButton("V1 - Find Standard Deviation", Color.ORANGE),
            createStyledButton("V1 - Find Variance", Color.CYAN),
            createStyledButton("V1 - Find Factorial", Color.PINK),
            createStyledButton("V1 - Find Permutation", Color.MAGENTA),
            createStyledButton("V1 - Find Combination", Color.YELLOW),
            createStyledButton("V1 - Determine Correlation", Color.LIGHT_GRAY),
            createStyledButton("V1 - Is Dependent?", Color.GRAY),
            createStyledButton("V1 - Find Union", Color.MAGENTA),
            createStyledButton("V1 - Find Intersection", Color.BLUE),
            createStyledButton("V1 - Find Compliment", Color.RED),
            createStyledButton("V1 - Conditional Probability", Color.GREEN),
            createStyledButton("V1 - Bayes' Theorem", Color.ORANGE),
            createStyledButton("V1 - Binomial Distribution", Color.CYAN),
            createStyledButton("V1 - Geometric On or Before N", Color.PINK),
            createStyledButton("V1 - Geometric Success Before N", Color.MAGENTA),
            createStyledButton("V1 - Geometric On or After N", Color.YELLOW),
            createStyledButton("V1 - Geometric Success After N", Color.LIGHT_GRAY),
            createStyledButton("V1 - Hypergeometric Distribution", Color.GRAY),
            createStyledButton("V1 - Negative Binomial Distribution", Color.GREEN),
            createStyledButton("V1 - Poisson Distribution", Color.BLUE),
            createStyledButton("V1 - Chebyshev's Theorem", Color.RED),
            createStyledButton("V2 - Uniform Distribution", Color.GREEN),
            createStyledButton("V2 - Joint Probability", Color.PINK),
            createStyledButton("V2 - Marginal Probability Dist.", Color.YELLOW),
            createStyledButton("V2 - Conditional Probability Dist.", Color.ORANGE)
        };
    ...
    private void handleButtonClick(int index) 
    {
        switch (index) 
        {
            ...
            case 23:
                ...
                break;
            case 24:
            	showUniformDistributionInputDialog();
            	break;
            case 25:
            	showJointProbabilityInputDialog();
            	break;
            case 26:
            	showMarginalInputDialog();
            	break;
            case 27:
            	showConditionalInputDialog();
            	break;
            default:
                break;
        }
    ...
    // Call Uniform Distribution method and display
    private void showUniformDistributionInputDialog() 
    {
        JPanel inputPanel = new JPanel();
        inputPanel.setLayout(new GridLayout(2, 2));
        ...
        int result = JOptionPane.showConfirmDialog(null, inputPanel, "Uniform Distribution", JOptionPane.OK_CANCEL_OPTION);
        ...
        if (result == JOptionPane.OK_OPTION) 
        {
            ...
            // Call the method and display the results
            double uniformProbability = statsLibrary.uniformProbability(lowerBound, upperBound);
            displayResult("Uniform Probability Distribution", uniformProbability);
        }
    }
    
    // Call Joint Probability Distribution method and display
    private void showJointProbabilityInputDialog() 
    {
        JPanel inputPanel = new JPanel();
        inputPanel.setLayout(new GridLayout(2, 2));
        JTextField probabilitiesAField = new JTextField(20);
        JTextField probabilitiesBField = new JTextField(20);

        // Get user input
        inputPanel.add(new JLabel("Enter probabilities for Y1 (comma-separated): "));
        inputPanel.add(probabilitiesAField);
        inputPanel.add(new JLabel("Enter probabilities for Y2 (comma-separated): "));
        inputPanel.add(probabilitiesBField);

        int result = JOptionPane.showConfirmDialog(null, inputPanel, "Enter Probabilities", JOptionPane.OK_CANCEL_OPTION);

        if (result == JOptionPane.OK_OPTION) 
        {
        	
            // Get user input for probabilities A
            String[] probabilitiesAStrings = probabilitiesAField.getText().split(",");
            ArrayList<Double> probabilitiesA = new ArrayList<>();
            
            for (String probabilityString : probabilitiesAStrings) 
            {
                try {
                    probabilitiesA.add(Double.parseDouble(probabilityString.trim()));
                } catch (NumberFormatException e) {
                    JOptionPane.showMessageDialog(null, "Invalid input for PA.", "User Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }

            // Get user input for probabilities B
            String[] probabilitiesBStrings = probabilitiesBField.getText().split(",");
            ArrayList<Double> probabilitiesB = new ArrayList<>();
            for (String probabilityString : probabilitiesBStrings) 
            {
                try {
                    probabilitiesB.add(Double.parseDouble(probabilityString.trim()));
                } catch (NumberFormatException e) {
                    JOptionPane.showMessageDialog(null, "Invalid input for PA.", "User Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            ...
            // Call the StatsLibrary method to calculate the joint probability
            double[][] jointProbability = statsLibrary.findJointProbability(probabilitiesA, probabilitiesB);

            // Display the result
            displayJointProbability(jointProbability);
        }
    }
    
    // Display Joint Probability
    private void displayJointProbability(double[][] jointProbability) 
    {
        // Display the joint probabilities
        JTextArea resultTextArea = new JTextArea();
        for (int i = 0; i < jointProbability.length; i++) 
            for (int j = 0; j < jointProbability[i].length; j++) 
                resultTextArea.append(String.format("P(Y1=%d, Y2=%d) = %.4f%n", i, j, jointProbability[i][j]));

        // Show the result on a JOptionPane
        JOptionPane.showMessageDialog(null, new JScrollPane(resultTextArea), "Joint Probability Result", JOptionPane.INFORMATION_MESSAGE);
    }
    
    // Marginal Probability Distribution
    private void showMarginalInputDialog() 
    {
        JPanel inputPanel = new JPanel();
        inputPanel.setLayout(new GridLayout(2, 1));
        JTextField probabilitiesField = new JTextField(20);
        JTextField variableIndexField = new JTextField(5);

        // Get user input
        inputPanel.add(new JLabel("Enter probabilities (comma-separated): "));
        inputPanel.add(probabilitiesField);
        inputPanel.add(new JLabel("Enter variable index: "));
        inputPanel.add(variableIndexField);
        ...
        if (result == JOptionPane.OK_OPTION) 
        {
            try 
            {
                ...
                String[] probabilitiesStrings = probabilitiesField.getText().split(",");
                ArrayList<Double> probabilities = new ArrayList<>();
                for (String probabilityString : probabilitiesStrings)
                    probabilities.add(Double.parseDouble(probabilityString.trim()));

                // Get user input for variable index (in textbook)
                int variableIndex = Integer.parseInt(variableIndexField.getText().trim());

                // Call the method and display the result
                double marginalProbability = statsLibrary.findMarginalProbability(probabilities, variableIndex);
                displayResult("Marginal Probability", marginalProbability);
            } 
            catch (IllegalArgumentException e) 
            {
                JOptionPane.showMessageDialog(null, "Invalid input.", "User Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    
    // Conditional Probability Distribution
    private void showConditionalInputDialog() 
    {
        JPanel inputPanel = new JPanel();
        inputPanel.setLayout(new GridLayout(3, 2));

        JTextField jointProbabilitiesField = new JTextField(20);
        JTextField marginalProbabilitiesField = new JTextField(20);
        JTextField givenIndexField = new JTextField(10);

        inputPanel.add(new JLabel("Enter joint probabilities (comma-separated): "));
        inputPanel.add(jointProbabilitiesField);
        inputPanel.add(new JLabel("Enter marginal probabilities (comma-separated): "));
        inputPanel.add(marginalProbabilitiesField);
        inputPanel.add(new JLabel("Enter given variable index: "));
        inputPanel.add(givenIndexField);

        int result = JOptionPane.showConfirmDialog(null, inputPanel, "Conditional Probability Distribution", JOptionPane.OK_CANCEL_OPTION);

        if (result == JOptionPane.OK_OPTION) 
        {
            // Get user input for joint probabilities
            String[] jointProbabilitiesStrings = jointProbabilitiesField.getText().split(",");
            ArrayList<Double> jointProbabilities = new ArrayList<>();
            
            for (String probabilityString : jointProbabilitiesStrings) 
            {
                try 
                {
                    jointProbabilities.add(Double.parseDouble(probabilityString.trim()));
                } catch (NumberFormatException e) 
                {
                    JOptionPane.showMessageDialog(null, "Invalid user input.", "User Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }

            // Get user input for marginal probabilities
            String[] marginalProbabilitiesStrings = marginalProbabilitiesField.getText().split(",");
            ArrayList<Double> marginalProbabilities = new ArrayList<>();
            
            for (String probabilityString : marginalProbabilitiesStrings) 
            {
                try 
                {
                    marginalProbabilities.add(Double.parseDouble(probabilityString.trim()));
                } catch (NumberFormatException e) 
                {
                    JOptionPane.showMessageDialog(null, "Invalid user input for marginal probabilities, please make sure your numbers are valid.", "User Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }

            // Get user input for given variable index
            int givenIndex;
            
            try 
            {
                givenIndex = Integer.parseInt(givenIndexField.getText().trim());
            } catch (NumberFormatException e) 
            {
                JOptionPane.showMessageDialog(null, "Invalid user input.", "User Error", JOptionPane.ERROR_MESSAGE);
                return;
            ...
            try 
            {
                double conditionalProbability = statsLibrary.findConditionalProbability(jointProbabilities, marginalProbabilities, givenIndex);
                displayResult("Conditional Probability Distribution Result", conditionalProbability);
            } catch (IllegalArgumentException e) 
            {
