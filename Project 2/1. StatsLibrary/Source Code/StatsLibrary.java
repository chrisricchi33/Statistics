...
public class StatsLibrary 
{
    ...
    public double uniformProbability(double lowerBound, double upperBound) 
    {
        if (lowerBound >= upperBound)
            throw new IllegalArgumentException("Invalid uniform bounds.");

        // Calculate the probability by integrating the PDF
        double probability = integrateUniformPDF(lowerBound, upperBound);

        return probability;
    }

    // Uniform Distribution Integration
    private double integrateUniformPDF(double lowerBound, double upperBound) 
    {
        // Integration of (x-a) / (b-a) evaluated from a to b
        double result = (upperBound - lowerBound) / (upperBound - lowerBound);
        return result;
    }
    
    // Joint Probability Function
    public double[][] findJointProbability(ArrayList<Double> probabilitiesA, ArrayList<Double> probabilitiesB) 
    {
        int sizeA = probabilitiesA.size();
        int sizeB = probabilitiesB.size();

        // Check if the sizes of the probability distributions match
        if (sizeA != sizeB) 
            throw new IllegalArgumentException("Sizes of probability distributions do not match.");

        // Create a 2D array to store the joint probabilities
        double[][] jointProbability = new double[sizeA][sizeB];

        // Calculate the joint probability for each combination of Y1 and Y2
        for (int i = 0; i < sizeA; i++) 
            for (int j = 0; j < sizeB; j++) 
                jointProbability[i][j] = probabilitiesA.get(i) * probabilitiesB.get(j);

        return jointProbability;
    }
    
    // Marginal Probability Function
    public double findMarginalProbability(ArrayList<Double> probabilities, int variableIndex) 
    {
        int size = probabilities.size();

        // Check if the variable index is valid
        if (variableIndex < 0 || variableIndex >= size)
            throw new IllegalArgumentException("Invalid variable index.");

        // Calculate the marginal probability for the specified variable
        double marginalProbability = 0.0;
        
        for (int i = 0; i < size; i++)
            if (variableIndex == i)
                marginalProbability += probabilities.get(i);

        return marginalProbability;
    }
    
    // Conditional Probability Function for Discrete Random Variables
    public double findConditionalProbability(ArrayList<Double> jointProbabilities, ArrayList<Double> marginalProbabilities, int givenIndex) {
        int size = jointProbabilities.size();

        // Check if the given index is valid
        if (givenIndex < 0 || givenIndex >= size)
            throw new IllegalArgumentException("Invalid given index.");

        // Calculate the conditional probability for the specified variable
        double marginalProbability = marginalProbabilities.get(givenIndex);

        // Check if the marginal probability is not zero
        if (marginalProbability == 0)
            throw new IllegalArgumentException("Marginal probability is zero.");

        // Calculate the conditional probability
        return jointProbabilities.get(givenIndex) / marginalProbability;
    }
}
